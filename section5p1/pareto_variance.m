...
bar_pi_list =...
    [min_pi:(delta_pi/15):(min_pi+(2*delta_pi)),...
    (min_pi+(3*delta_pi)):delta_pi:max_pi];
for iP = 1:length(bar_pi_list)
    bar_pi = bar_pi_list(iP);
...
if Flag.save == 1
    saveas(gcf, strcat(filename.std,'_varia_Pareto_1_alpha',num2str(param.alpha)), 'epsc');
end
...
if Flag.save == 1
    saveas(gcf, strcat(filename.std,'_varia_Pareto_2_alpha',num2str(param.alpha)), 'epsc');
end
...
ylabel('Discrepancy of compliance (J)', 'Interpreter', 'latex');
if Flag.save == 1
    saveas(gcf, strcat(filename.std,'_varia_Pareto_3_alpha',num2str(param.alpha)), 'epsc');
end



figure;
plot(his_w_mean, his_w_quadVar, 'g-', 'LineWidth',1.5);
hold on;
xlim([360, 400]);
ylim([1450, 2200]);
set(gca,'FontName','Times New Roman');
...
if Flag.save == 1
    saveas(gcf, strcat(filename.std,'_varia_Pareto_4_alpha',num2str(param.alpha)), 'epsc');
end
